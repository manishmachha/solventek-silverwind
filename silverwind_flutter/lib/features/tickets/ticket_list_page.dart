...
class _TicketListPageState extends ConsumerState<TicketListPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final result = await svc.getMyTickets();
      if (mounted) {
        setState(() {
          if (result is List) {
            _items = result;
            _totalElements = result.length;
          } else if (result is Map<String, dynamic> && result.containsKey('content')) {
        ...
        children: [
          ...
          Row(children: [
            Container(
              ...
              child: Icon(Icons.confirmation_number, color: Colors.white, size: 28),
            ),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Tickets', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
              Text('$_totalElements total tickets', style: const TextStyle(fontSize: 14, color: AppColors.textMuted)),
            ])),
          ]),
          ...
          Expanded(child: _loading
            ...
            : filtered.isEmpty
              ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  const SizedBox(height: 16),
                  Text('No tickets found', style: TextStyle(fontSize: 16, color: AppColors.textMuted, fontWeight: FontWeight.w500)),
                ]))
              : Container(
  ...
  Widget _buildListItem(Map<String, dynamic> item) {
    ...
    return ListTile(
      ...
      onTap: id.isNotEmpty ? () => context.go('/portal/tickets/$id') : null,
    );
  }
