...
class ApprovedOrgsPage extends ConsumerStatefulWidget {
  const ApprovedOrgsPage({super.key});
  @override
  ConsumerState<ApprovedOrgsPage> createState() => _ApprovedOrgsPageState();
}

class _ApprovedOrgsPageState extends ConsumerState<ApprovedOrgsPage> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    ...
    try {
      final svc = ref.read(organizationServiceProvider);
      final result = await svc.getApprovedOrganizations();
      if (mounted) {
        setState(() {
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Organization Discovery', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
            Text('${_items.length} organizations', style: const TextStyle(fontSize: 14, color: AppColors.textMuted)),
          ])),
        ...
        Expanded(child: _loading
          ...
          : _items.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No organizations found', style: TextStyle(fontSize: 16, color: AppColors.textMuted)),
              ]))
            : Container(
