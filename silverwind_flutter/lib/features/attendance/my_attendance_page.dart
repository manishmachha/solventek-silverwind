...
class _MyAttendancePageState extends ConsumerState<MyAttendancePage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final result = await svc.getMyAttendance();
      if (mounted) {
        setState(() {
          _items = result is List ? result : (result is Map && (result as Map).containsKey('content') ? (result['content'] as List? ?? []) : []);
          _loading = false;
        });
      }
    } catch (e) {
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('My Attendance', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
            Text('${_items.length} records', style: const TextStyle(fontSize: 14, color: AppColors.textMuted)),
          ])),
        ...
        Expanded(child: _loading
          ...
          : _items.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No records found', style: TextStyle(fontSize: 16, color: AppColors.textMuted)),
              ]))
            : Container(
