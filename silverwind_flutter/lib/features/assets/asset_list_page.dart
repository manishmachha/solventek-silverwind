...
class _AssetListPageState extends ConsumerState<AssetListPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final result = await svc.listAssets();
      if (mounted) {
        setState(() {
        ...
        children: [
          ...
          Row(children: [
            Container(
              ...
              child: Icon(Icons.laptop, color: Colors.white, size: 28),
            ),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Asset Management', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
              Text('$_totalElements total assets', style: const TextStyle(fontSize: 14, color: AppColors.textMuted)),
            ])),
          ]),
          ...
          Expanded(child: _loading
            ...
            : filtered.isEmpty
              ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  const SizedBox(height: 16),
                  Text('No assets found', style: TextStyle(fontSize: 16, color: AppColors.textMuted, fontWeight: FontWeight.w500)),
                ]))
              : Container(
  ...
  Widget _buildListItem(Map<String, dynamic> item) {
    ...
    return ListTile(
      ...
      onTap: id.isNotEmpty ? () => context.go('/admin/assets/$id') : null,
    );
  }
