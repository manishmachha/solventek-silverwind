...
class LeaveConfigurationPage extends ConsumerStatefulWidget {
  const LeaveConfigurationPage({super.key});
  @override
  ConsumerState<LeaveConfigurationPage> createState() => _LeaveConfigurationPageState();
}

class _LeaveConfigurationPageState extends ConsumerState<LeaveConfigurationPage> {
  List<dynamic> _items = [];
  bool _loading = true;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final result = await svc.getLeaveTypes();
      if (mounted) {
        setState(() {
      ...
      body: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Text('Leave Configuration', style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
            Text('${_items.length} leave types', style: const TextStyle(fontSize: 14, color: AppColors.textMuted)),
          ])),
        ...
        Expanded(child: _loading
          ...
          : _items.isEmpty
            ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                ...
                Text('No leave types found', style: TextStyle(fontSize: 16, color: AppColors.textMuted)),
              ]))
            : Container(
