...
class _AuditLogListPageState extends ConsumerState<AuditLogListPage> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      final svc = ref.read(auditLogServiceProvider);
      final result = await svc.getAuditLogs();
      if (mounted) {
        setState(() {
        ...
        children: [
          ...
          Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Audit Logs', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
              Text('$_totalElements total logs', style: const TextStyle(fontSize: 14, color: AppColors.textMuted)),
            ])),
          ]),
          ...
          Expanded(child: _loading
            ...
            : filtered.isEmpty
              ? Center(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                  ...
                  const SizedBox(height: 16),
                  Text('No logs found', style: TextStyle(fontSize: 16, color: AppColors.textMuted, fontWeight: FontWeight.w500)),
                ]))
              : Container(
  ...
  Widget _buildListItem(Map<String, dynamic> item) {
    ...
    return ListTile(
      ...
      onTap: id.isNotEmpty ? () => context.go('/admin/audit-logs/$id') : null,
    );
  }
