...
class PayrollManagementPage extends ConsumerStatefulWidget {
  const PayrollManagementPage({super.key});
  @override
  ConsumerState<PayrollManagementPage> createState() => _PayrollManagementPageState();
}

class _PayrollManagementPageState extends ConsumerState<PayrollManagementPage> {
  List<dynamic> _items = [];
  bool _loading = true;
  String _searchQuery = '';
  int _currentPage = 0;
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final result = await svc.getPayrollRecords();
      if (mounted) {
        setState(() {
        ...
        children: [
          ...
          Row(children: [
            Container(
              ...
              child: Icon(Icons.currency_rupee, color: Colors.white, size: 28),
            ),
            const SizedBox(width: 16),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('Payroll Management', style: TextStyle(fontSize: 26, fontWeight: FontWeight.w700, color: AppColors.textMain)),
  ...
  Widget _buildListItem(Map<String, dynamic> item) {
    ...
    return ListTile(
      ...
      onTap: id.isNotEmpty ? () => context.go('/admin/payroll/$id') : null,
    );
  }
